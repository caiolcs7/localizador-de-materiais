...
const recentKey='lm-recent:v2'; const legacyRecentKey='lm-recent'; const favoriteKey='lm-favorites'
const loadList=(key:string)=>{try{const value=JSON.parse(localStorage.getItem(key)||'[]');return Array.isArray(value)?value.filter((item):item is string=>typeof item==='string'):[]}catch{return []}}
const saveList=(key:string,value:string[])=>{try{localStorage.setItem(key,JSON.stringify(value))}catch{return}}
const removeList=(key:string)=>{try{localStorage.removeItem(key)}catch{return}}
const loadInitialRecentSearches=()=>{const current=loadRecentSearches(recentKey);return current.length?current:loadRecentSearches(legacyRecentKey)}
...
export default function App({ adminMode=false, adminEmail, onLogout }: AppProps) {
  ...
  useEffect(()=>{document.documentElement.dataset.theme=dark?'dark':'light';try{localStorage.setItem('lm-theme',dark?'dark':'light')}catch{return}},[dark])
  useEffect(()=>{if(!ready){return};let active=true;const id=setTimeout(async()=>{const r=await searchInventory(query);if(!active)return;setResult(r);if(query.trim()&&isCompleteSearch(r)){setRecent(prev=>{const n=mergeRecentSearch(prev,query);saveList(recentKey,n);return n})}},180);return()=>{active=false;clearTimeout(id)}},[query,ready])
  useEffect(()=>{if(!ready)return;let active=true;const stored=initialRecentRef.current;void Promise.all(stored.map(async value=>({value,result:await searchInventory(value)}))).then(entries=>{if(!active)return;const storedKeys=new Set(stored.map(normalizeSearch));const validStored=entries.filter(entry=>isCompleteSearch(entry.result)).map(entry=>entry.value);setRecent(current=>{const searchesAddedMeanwhile=current.filter(value=>!storedKeys.has(normalizeSearch(value)));const cleaned=deduplicateRecentSearches([...searchesAddedMeanwhile,...validStored]);saveList(recentKey,cleaned);removeList(legacyRecentKey);return JSON.stringify(current)===JSON.stringify(cleaned)?current:cleaned})});return()=>{active=false}},[ready])
  ...
  const toggleFav=(s:string)=>setFavorites(prev=>{const n=prev.includes(s)?prev.filter(x=>x!==s):[s,...prev].slice(0,12);saveList(favoriteKey,n);return n})
