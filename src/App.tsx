...
export default function App({ adminMode=false, adminEmail, onLogout }: AppProps) {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {query.trim() && result.items.length===0 && <div className="empty-search"><b>Código não encontrado</b><span>{query}</span>{result.suggestion&&<button onClick={()=>setQuery(result.suggestion!)}>Você quis dizer <b>{result.suggestion}</b>?</button>}{adminMode&&<button className="primary-button" onClick={()=>setEditor({codigo:query})}>Cadastrar este código</button>}</div>}
    ...
    {scanner&&<ScannerModal onDetected={detect} onClose={()=>setScanner(false)}/>} {adminMode&&editor&&<ItemModal initial={editor} onClose={()=>setEditor(null)} onSaved={async()=>{await refresh();if(query)setResult(await searchInventory(query));notify(editor.id?'Alterações salvas':'Item salvo')}}/>} {toast&&<div className="toast">{toast}</div>}
  </div>
}
